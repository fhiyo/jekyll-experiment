...
## common lispのnthが線形なので別の関数はあるか？
↓  
listのconstant searchは普通に考えてないだろう．  
**listとarrayは違う．**  
arrayを作るには`make-array`関数を使い，配列にアクセスするには`aref`関数を使う．  
これは定数時間でいけるようだ．

**しかも，ハッシュテーブルは`alist`とは違うらしい．ハッシュテーブルの方が要素のアクセスが速い．(Land Of Lisp, p.150 9.2. ハッシュテーブル 参照)**



## Nonceとは

何回覚えようとしても忘れるのでここにメモする．

[nonce：リプレイ攻撃に対する対処方法](https://thinkit.co.jp/article/839/1)
...
## Macでghciでctrl+yがsuspend信号を送る

```sh
stty dsusp undef
```

を設定すればOK.自分はzsh使っているので，`.zshrc`に上の記述を追加した．
